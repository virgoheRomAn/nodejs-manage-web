...
import { verifyUrl } from '/@/plugins/verify';
...
const onGotoFullPage = () => {
	...
	if (verifyUrl(state.isLink as string)) window.open(state.isLink);
...
watch(
	() => route.path,
	() => {
		state.title = route.meta.title as string;
		state.isLink = route.meta.isLink as string;
	},
	{ immediate: true }
);
</script>
