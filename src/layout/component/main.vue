<template>
	<el-main class="layout-main" :style="isFixedHeader ? `height: calc(100% - ${setMainHeight})` : `minHeight: calc(100% - ${setMainHeight})`">
		<el-scrollbar
			...
			:wrap-class="`layout-main-scroll ${!isUseScrollBar ? 'overflow-hidden' : ''}`"
			:view-class="`layout-main-scroll ${!isUseScrollBar ? 'overflow-hidden' : ''}`"
		>
			<LayoutParentView :class="{ h100: !isUseScrollBar }" />
...
import { defineAsyncComponent, onMounted, computed, ref, provide } from 'vue';
...
// 定义变量内容
const isUseScrollBar = ref(false);
...
const setMainHeight = computed(() => {
	...
	if (isTagsview && layout !== 'classic') return '90px';
	else return '56px';
});

// 注入是否需要滚动
const setScrollBar = (val: boolean) => {
	isUseScrollBar.value = val;
};
provide('useScrollBar', { isUseScrollBar, setScrollBar });
