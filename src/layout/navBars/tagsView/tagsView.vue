...
const setTagsViewNameI18n = computed(() => (v: RouteItem) => settings.setTagsViewNameI18n(v));
...
onBeforeRouteUpdate(async (to) => {
	...
	await addTagsView(to.path, to as RouteToFrom);
...
const initTagsView = async () => {
	if (session.get('tagsViewList') && getThemeConfig.value.isCacheTagsView) {
		...
		await addTagsView(route.path, route as RouteToFrom);
	}
	// 初始化当前元素(li)的下标
	getTagsRefsIndex(getThemeConfig.value.isShareTagsView ? state.routePath : state.routeActive);
};
...
const closeOtherTagsView = (path: string) => {
	if (session.get('tagsViewList')) {
		state.tagsViewList = [];
		session.get('tagsViewList').forEach((v: RouteItem) => {
			if (v.meta?.isAffix && !v.meta.isHide) {
				v.url = setTagsViewHighlight(v);
		...
		addTagsView(path, route as RouteToFrom);
		addBrowserSetSession(state.tagsViewList);
	}
};
...
const onMousedownMenu = (v: RouteItem, e: MouseEvent) => {
	if (!v.meta?.isAffix && e.button === 1) {
		const item = { contextMenuClickId: 1, ...v };
...
const onSortableResize = async () => {
	await initSortable();
	if (U.browser.isMobile) state.sortable.el && state.sortable.destroy();
};
</script>
