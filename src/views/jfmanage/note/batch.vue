<template>
	<div class="vhm-layout-container">
		<LayoutContainer title="批次查询" :scrollBarCls="isPageTableFixed ? 'is-table-fixed' : ''" isFixed :loading="loading">
			<template #default>
				<ListContent
					isTableHeader
...
<script setup lang="ts" name="jfManage.note.batch">
import { C } from '/@/dicts';
import { U } from '/@/utils';
...
import { queryBatchSms } from '/@/api/jfmanage/note';
...
const form = reactive({
	...
	showFields: {
		mainRecordId: { key: '批次号', value: '' },
		templateContent: { key: '模板内容', value: '' },
...
const table = reactive({
	...
	showFields: {
		channelName: { key: '平台方', width: '200px' },
		mainRecordId: { key: '系统批次号', width: '200px' },
		templateContent: { key: '模板内容' },
		count: { key: '统计', width: '100px' },
		commitStatus: { key: '提交状态', width: '100px', convert: C.jfManageDict.NoteBatchSendStatus, tag: true },
		createTime: { key: '提交时间', width: '200px', timestamp: true }
	}
});
...
async function initQuery(data?: EmptyObjectType, loading: boolean = true) {
	const rs: any = await queryBatchSms({ ...data, pages: form.pages }, loading);
	if (rs.code === 'SUCCESS') {
		table.list = rs.data;
