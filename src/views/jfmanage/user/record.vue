...
<script setup lang="ts" name="jfManage.user.record">
import { C } from '/@/dicts';
import { U } from '/@/utils';
...
import { queryCustomerInfo } from '/@/api/jfmanage/user';
...
const form = reactive({
	...
	showFields: {
		...
		customerIdNo: { key: '身份证号', value: '' },
		customerMobile: { key: '手机号码', value: '' }
	},
...
const table = reactive({
	...
	showFields: {
		...
		customerIdNo: { key: '身份证号', AES: true },
		customerMobile: { key: '手机号码', AES: true }
	}
});
...
async function initQuery(data?: EmptyObjectType, loading: boolean = true) {
	const rs: any = await queryCustomerInfo({ ...data, pages: form.pages }, loading);
	if (rs.code === 'SUCCESS') {
		table.list = rs.data;
