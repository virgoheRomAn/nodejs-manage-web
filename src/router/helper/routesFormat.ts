...
/**
 * @description 一维数组处理成多级嵌套数组（只保留二级：也就是二级以上全部处理成只有二级，keep-alive 支持二级缓存）
 ...
 * @returns 返回替换后的路由数组
 */
export function setFilterRouteEnd(routes: Array<any>) {
	let filterRouteEnd: any = formatTwoStageRoutes(formatFlatteningRoutes(routes));
