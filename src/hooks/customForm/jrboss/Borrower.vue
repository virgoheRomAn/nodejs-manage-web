...
<script setup lang="ts" name="jrBossCustomFormBorrower">
import { U } from '/@/utils';
import { C } from '/@/dicts';
import { settings } from '/@/plugins/settings';
import { RenderBorrowerFieldsInit } from './renderFields';
...
const form = reactive({
	showFields: RenderBorrowerFieldsInit(editor.value, props.routeParams),
	...
	nextTick(() => {
		switch (key) {
			case 'tableAdd':
				...
				break;
			case 'tableDetails':
				title.value = `查看共借人 - ${row.name}`;
				break;
			case 'tableChange':
				title.value = `编辑共借人 - ${row.name}`;
				break;
		}
		...
		if (!isDelete) {
			...
			jBox.confirm(`是否确认删除共借人 「${row.name}」?`, '删除操作', {
