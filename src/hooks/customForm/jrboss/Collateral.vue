...
<script setup lang="ts" name="jrBossCustomFormCollateral">
import { U } from '/@/utils';
import { C } from '/@/dicts';
import { settings } from '/@/plugins/settings';
import { RenderCollateralFieldsInit } from './renderFields';
...
const params = { ...props.routeParams, ...{ childrenHandler } };
const form = reactive({
	showFields: RenderCollateralFieldsInit(editor.value, params),
	...
	nextTick(() => {
		switch (key) {
			case 'tableAdd':
				...
				break;
			case 'tableDetails':
				title.value = `查看抵押物小区 - ${row.communityName}`;
				break;
			case 'tableChange':
				title.value = `编辑抵押物小区 - ${row.communityName}`;
				break;
		}
		...
		if (!isDelete) {
			...
			jBox.confirm(`是否确认删除小区 「${row.communityName}」?`, '删除操作', {
...
function childrenHandler() {
	const owers = U.clone(form.formData['orderCollateralOwner']);

	if (owers.length >= 5) {
		jBox.error('一个抵押物最多只能新增五个归属人');
		return false;
	}

	const orderCollateralOwner = form.showFields['orderCollateralOwner'];
	const { formatField, value } = orderCollateralOwner;
	const finallyField = U.clone(formatField);
	const finallyFieldForm = U.formatFormData(finallyField, { cutting: true, submit: true });
	(value as any[]).push(finallyField);
	owers.push(U.clone(finallyFieldForm));

	form.formData['orderCollateralOwner'] = owers;
}
