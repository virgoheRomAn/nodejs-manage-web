...
module.exports = (appInfo) => {
	...
	config.static = {
		prefix: '/assets',
		dir: [path.join(appInfo.baseDir, 'dist/assets'), { prefix: '/resource', dir: path.join(appInfo.baseDir, 'dist/resource') }]
	};
	...
	config.logger = {
		...
		contextFormatter(meta) {
			...
			const logHead = `${meta.date.replace(/,/gi, '.')} [${meta.level}] [${appInfo.name},${header['x-b3-traceid'] || ''},${
				header['x-b3-spanid'] || ''
			}]`;
